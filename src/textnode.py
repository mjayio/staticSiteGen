...
class NodeType(Enum):
    """
    NodeType is an enumeration that defines different types of nodes that can be used in a static site generator.
...
class TextNode:
    ...
    Methods:
    --------
    __init__(text: str, text_type_enum: NodeType, url: Optional[str] = None):
    ...
    """
    def __init__(self, text: str, text_type_enum: NodeType, url: Optional[str] = None):
        if not isinstance(text_type_enum, NodeType):
            raise ValueError("text_type_enum must be an instance of NodeType")
        self.text = text
